		...
		"ng_no_fbs": {
			PFD: &pfd.PFD{
				...
				Edges: sets.New(
					...
					&pfd.Edge{Source: "P1", Target: "P2"},
					&pfd.Edge{Source: "P2", Target: "D2"},
				),
			},
			Expected: []checkers.Problem{checkers.NewProblem("no-p2p", checkers.SeverityError, pfdcommon.NewLocations(pfdcommon.NewLocation(pfdcommon.LocationTypePFD, "P1", "P2"))...)},
		},
		"ok_no_fbs": {
			PFD: &pfd.PFD{
				Nodes: sets.New(
		...
		"ng_fbs": {
			PFD: &pfd.PFD{
				...
				Edges: sets.New(
					...
					&pfd.Edge{Source: "D2", Target: "P2"},
					&pfd.Edge{Source: "P2", Target: "D3"},
					&pfd.Edge{Source: "P2", Target: "P1", IsFeedback: true},
				),
			},
			Expected: []checkers.Problem{checkers.NewProblem("no-p2p", checkers.SeverityError, pfdcommon.NewLocations(pfdcommon.NewLocation(pfdcommon.LocationTypePFD, "P2", "P1"))...)},
		},
		"ok_fbs": {
			PFD: &pfd.PFD{
				Nodes: sets.New(
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			go func() {
				...
				if err := NoP2P.Check(tgt, ch); err != nil {
					t.Errorf("NoP2P.Check: %v", err)
				}
			}()
			got := chans.Slice(ch)

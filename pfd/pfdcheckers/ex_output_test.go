...
func TestExOutput(t *testing.T) {
	testCases := map[string]struct {
		...
		"ng": {
			...
			Expected: []checkers.Problem{checkers.NewProblem("ex-output", checkers.SeverityError, pfdcommon.NewLocations(pfdcommon.NewLocation(pfdcommon.LocationTypePFD, "P1"))...)},
		},
		"ok": {
			PFD: &pfd.PFD{
				Nodes: sets.New(
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			go func() {
				...
				if err := ExOutput.Check(tgt, ch); err != nil {
					t.Errorf("ExOutput.Check: %v", err)
				}
			}()
			got := chans.Slice(ch)

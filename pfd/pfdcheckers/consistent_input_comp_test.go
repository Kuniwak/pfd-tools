...
func TestConsistentInputComp(t *testing.T) {
	testCases := map[string]struct {
		...
		"ng-missing": {
			...
			Expected: []checkers.Problem{checkers.NewProblem("consistent-input-comp", checkers.SeverityError, pfdcommon.NewLocation(pfdcommon.LocationTypePFD, "D2", "P3"))},
		},
		"ok-missing": {
			PFD: &pfd.PFD{
				Nodes: sets.New(
		...
		"ng-extra": {
			PFD: &pfd.PFD{
				...
				ProcessComposition: map[pfd.NodeID]*sets.Set[pfd.NodeID]{
					"P4": sets.New(pfd.NodeID.Compare, "P2", "P3"),
				},
			},
			Expected: []checkers.Problem{checkers.NewProblem("consistent-input-comp", checkers.SeverityError, pfdcommon.NewLocation(pfdcommon.LocationTypePFD, "D1", "P4"))},
		},
		"ok-extra": {
			PFD: &pfd.PFD{
				Nodes: sets.New(
		...
		"ok-composite-deliverable": {
			PFD: &pfd.PFD{
				...
				Edges: sets.New(
					...
					&pfd.Edge{Source: "D1", Target: "P1"},
					&pfd.Edge{Source: "D2", Target: "P1"},
					&pfd.Edge{Source: "P1", Target: "D5"},
					&pfd.Edge{Source: "D3", Target: "P4"},
					&pfd.Edge{Source: "D1", Target: "P4"},
					&pfd.Edge{Source: "D2", Target: "P4"},
					&pfd.Edge{Source: "P4", Target: "D5"},
				),
				ProcessComposition: map[pfd.NodeID]*sets.Set[pfd.NodeID]{
		...
		"ng-composite-deliverable": {
			...
			Expected: []checkers.Problem{checkers.NewProblem("consistent-input-comp", checkers.SeverityError, pfdcommon.NewLocation(pfdcommon.LocationTypePFD, "D2", "P4"))},
		},
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			...
			go func() {
				...
				if err := ConsistentInputComp.Check(tgt, ch); err != nil {
					t.Errorf("ConsistentInputComp.Check: %v", err)
				}
			}()
			got := chans.Slice(ch)
